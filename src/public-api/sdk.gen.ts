 ...
 * 3. **Real Purchase Count:** Count of real purchases of this company’s products that informed this price estimate.
 * 4. **Similar Real Purchases:** We find real purchases similar to your requirement and rank them based on their overall similarity score, starting with the most similar going down to at least 50% similar. The similarity factors between your requirement and real purchase contracts we consider are:
 * 1. **Product Overlap:** How many of the same products appear in both
 * 2. **Dimension Matching:** How closely the product dimensions (like user counts, storage limits) align between both
 * 3. **Quantity Similarity:** How similar the quantities are for matching dimensions
 * 4. **Contract Term:** Contract length in both (12 months, 24 months, etc.)
 * 5. **Recency:** Recency of real purchases - newer purchases are given higher priority
 * 6. **TCV:** Similarity of Total Contract Value between both
 *
...
/**
 * List Intent
 * Use this endpoint to programmatically consume buyer intent signals from Vendr. The intent data available through this endpoint has 3 key parts:
 * 1. **Timestamp:** This is the exact time when the intent signal becomes available in the API. You can use this as a reliable cursor for pagination and polling, thereby ensuring that you don’t get duplicate signals or miss any signals. Please note that this timestamp is different from the time of user interaction which is part of event details. Use the fromTimestamp and toTimestamp filters to list intent signals for specific time periods, especially when you make recurring calls to the APIs to continuously receive fresh intent. For example, calling the APIs every hour to get intent signals.
 * 2. **User Details:** This object contains multiple optional fields such as `companyName`, `domain`, `ipAddress`. Any of these fields maybe available for an intent signal. In the query parameters, you have the option to filter signals based on whether one or more of these field is present or not. For example, adding the filter *`userHasCompanyDomain`* would give only those intent signals for which the field domain is populated.
 * 3. **Event Details:** This object contains one of the 4 possible user engagement types. All events contain fields to identify the engagement type, the time of engagement, name of the software seller (and in some case the product) the engagement was for and the origin venue of the engagement (e.g. Vendr website, AI chatbot etc.):
 * 1. **Catalog View:** When buyers are looking for pricing insights, they visit an application or website powered by Vendr’s APIs to view the pricing data for a seller. For example, the pricing page for a seller on Vendr website. This visit is recorded as intent to buy a product from that seller. Buyers don't need to login to Vendr website to view this page. This engagement also includes total daily visits - if a user visits a website multiple times within a day, it is counted as one intent signal and the total visits convey signal strength.
 * 2. **Catalog View Verified:** These are instances when buyers login to the application or website that shows them pricing data for a seller powered by Vendr’s APIs. This engagement also includes total daily visits - if a user visits a website multiple times within a day, it is counted as one intent signal and the total visits convey signal strength.
 * 3. **Price Estimate:** After visiting the application or website, the buyer may use the pricing calculator on the page to get a customized price estimate for their need. This experience is also powered by Vendr APIs. Buyers are required to login to view a price estimate, therefore a price estimate intent signal is always verified. This engagement may include the price estimate response Vendr provided to the user.
 * 4. **Price Check:** After visiting the application or website, the buyer may use price check feature powered by Vendr APIs to analyze a quote they have received from a seller. Buyers are required to login to request a price check, therefore a price check intent signal is always verified. This engagement may include the estimate Vendr provided to the user for the contract and contract attributes as purchase type (renewal or new), renewal date.
 */
...
/**
 * Get FAQs
 * Call this endpoint with a company ID to get company specific negotiation insights structured as FAQs.
 */
export const getFaqs = <ThrowOnError extends boolean = false>(
  options: Options<GetFaqsData, ThrowOnError>,
) => {
  return (options.client ?? _heyApiClient).get<
    GetFaqsResponses,
    GetFaqsErrors,
 ...
 * Upload a file using a presigned URL for document processing and analysis.
 */
...
/**
 * Chat with an agent
 * Use this endpoint to chat with an agent. This endpoint is currently in beta and is only available to a limited number of partners.
 */
export const chatWithAgent = <ThrowOnError extends boolean = false>(
  options: Options<ChatWithAgentData, ThrowOnError>,
) => {
  return (options.client ?? _heyApiClient).post<
    ChatWithAgentResponses,
    ChatWithAgentErrors,
